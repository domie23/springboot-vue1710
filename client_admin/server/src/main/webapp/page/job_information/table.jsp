...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="job_information" lay-filter="job_information"></table>

        <script type="text/html" id="toolbarDemo">
            <div class="layui-btn-container">

                                            <!--跨表按钮-->
                                                    {{# if(d.add_job_submission>0){ }}
                            <button class="layui-btn layui-btn-sm" lay-event="job_submission">上交作业</button>
                            {{# } }}
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/job_information/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/job_information/table', 'del') || $check_action('/job_information/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/job_information/table', 'add') || $check_action('/job_information/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/job_information/view', 'get') || $check_action('/job_information/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        ...
        , cols: [[
                ...
                field: 'student_name', width: 180, title: '学生姓名', sort: true
                                ...
                                , {
                field: 'job_title', width: 180, title: '作业标题', sort: true
                                                                                
            }
                                , {
                field: 'operation_attachment', width: 180, title: '作业附件', sort: true
                                                                    , templet: function (d) {
                        return "<div><a href=" + fullUrl(BaseUrl,d.operation_attachment) + " > 点击下载</a></div > "
                    }
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='student_name']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'job_title', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='job_title']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'operation_attachment', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='operation_attachment']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
                            ...
                            for (var i = 0; i < res.result.list.length; i++) {
                                res.result.list[i].add_job_submission = 1
                            }
                        }
                        
            if (user_group == "管理员" || $check_pay('/job_information/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/job_information/view', 'set') || $check_action('/job_information/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(job_information)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.job_information_id;
                    }
            
                                else if (obj.event === 'job_submission'){
                        sessionStorage.setItem('data', JSON.stringify(data))
                        window.location.href = "../job_submission/view_add.jsp?"
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('job_information', BaseUrl + '/api/job_information/del', 'job_information_id', request, resetName, resetSelect        )
