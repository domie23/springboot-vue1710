...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="achievement_information" lay-filter="achievement_information"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/achievement_information/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/achievement_information/table', 'del') || $check_action('/achievement_information/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/achievement_information/table', 'add') || $check_action('/achievement_information/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/achievement_information/view', 'get') || $check_action('/achievement_information/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        ...
        , cols: [[
                ...
                field: 'student_name', width: 180, title: '学生姓名', sort: true
                                ...
                                , {
                field: 'final_exam', width: 180, title: '期末成绩', sort: true
                                                                                
            }
                                , {
                field: 'peacetime_performance', width: 180, title: '平时成绩', sort: true
                                                                                
            }
                                , {
                field: 'total_score', width: 180, title: '总成绩', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='student_name']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'final_exam', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='final_exam']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'peacetime_performance', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='peacetime_performance']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'total_score', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='total_score']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/achievement_information/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/achievement_information/view', 'set') || $check_action('/achievement_information/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(achievement_information)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.achievement_information_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('achievement_information', BaseUrl + '/api/achievement_information/del', 'achievement_information_id', request, resetName, resetSelect        )
