...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="withdrawal_information" lay-filter="withdrawal_information"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/withdrawal_information/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/withdrawal_information/table', 'del') || $check_action('/withdrawal_information/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/withdrawal_information/table', 'add') || $check_action('/withdrawal_information/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/withdrawal_information/view', 'get') || $check_action('/withdrawal_information/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        ...
        , cols: [[
                ...
                field: 'number_of_participants', width: 180, title: '选课人数', sort: true
                                ...
                                , {
                field: 'reason_for_withdrawal', width: 180, title: '退课原因', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='number_of_participants']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'reason_for_withdrawal', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='reason_for_withdrawal']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/withdrawal_information/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/withdrawal_information/view', 'set') || $check_action('/withdrawal_information/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(withdrawal_information)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.withdrawal_information_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('withdrawal_information', BaseUrl + '/api/withdrawal_information/del', 'withdrawal_information_id', request, resetName, resetSelect        )
