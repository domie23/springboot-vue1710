...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="teacher_user" lay-filter="teacher_user"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/teacher_user/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/teacher_user/table', 'del') || $check_action('/teacher_user/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/teacher_user/table', 'add') || $check_action('/teacher_user/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/teacher_user/view', 'get') || $check_action('/teacher_user/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
        ...
        , cols: [[
            ...
            field: 'user_id', width: 200, title: '用户名', templet: function (d) {
                ...
                field: 'nickname', width: 150, title: '昵称', templet: function (d) {
                    ...
                    , {
                field: 'teachers_name', width: 180, title: '老师姓名', sort: true
                                                                                
            }
                                , {
                field: 'gender_of_teacher', width: 180, title: '老师性别', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='teachers_name']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'gender_of_teacher', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='gender_of_teacher']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/teacher_user/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/teacher_user/view', 'set') || $check_action('/teacher_user/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(teacher_user)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {
        ...
        //下拉框
                ...
                let gender_of_teacher_data = ['男','女'];
                                async function gender_of_teacher() {
                ...
                layui.form.on('select(gender_of_teacher)', function (data) {
                    request.gender_of_teacher = data.elem[data.elem.selectedIndex].text;
                })
                    gender_of_teacher()
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('teacher_user', BaseUrl + '/api/teacher_user/del', 'teacher_user_id', request, resetName, resetSelect                        , undefined, undefined,true
        )
