...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="course_scheduling_information" lay-filter="course_scheduling_information"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/course_scheduling_information/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/course_scheduling_information/table', 'del') || $check_action('/course_scheduling_information/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/course_scheduling_information/table', 'add') || $check_action('/course_scheduling_information/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/course_scheduling_information/view', 'get') || $check_action('/course_scheduling_information/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        elem: '#course_scheduling_information'
        , toolbar: true
        , url: url
        , headers: {
            'x-auth-token': token,
        ...
        , cols: [[
                ...
                field: 'lecturer', width: 180, title: '授课教师', sort: true
                            ...
                            function (d) {
                                ...
                                , {
                field: 'teachers_name', width: 180, title: '教师姓名', sort: true
                ...
                field: 'course_name', width: 180, title: '课程名称', sort: true
                                ...
                                , {
                field: 'class_weeks', width: 180, title: '上课周数', sort: true
                                                                                
            }
                                , {
                field: 'class_time', width: 180, title: '上课时间', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='lecturer']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'teachers_name', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='teachers_name']").css('display', 'none');
                    }
                        ...
                        $("[data-field='course_name']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'class_weeks', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='class_weeks']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'class_time', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='class_time']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/course_scheduling_information/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/course_scheduling_information/view', 'set') || $check_action('/course_scheduling_information/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(course_scheduling_information)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.course_scheduling_information_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('course_scheduling_information', BaseUrl + '/api/course_scheduling_information/del', 'course_scheduling_information_id', request, resetName, resetSelect        )
