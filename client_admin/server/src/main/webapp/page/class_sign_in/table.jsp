...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="class_sign_in" lay-filter="class_sign_in"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/class_sign_in/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/class_sign_in/table', 'del') || $check_action('/class_sign_in/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/class_sign_in/table', 'add') || $check_action('/class_sign_in/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/class_sign_in/view', 'get') || $check_action('/class_sign_in/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        ...
        , cols: [[
                ...
                field: 'student_name', width: 180, title: '学生姓名', sort: true
                                ...
                                , {
                field: 'sign_in_time', width: 180, title: '签到时间', sort: true
                                                            , templet: "<div>{{layui.util.toDateString(d.sign_in_time, 'yyyy-MM-dd HH:mm:ss')}}</div>"
                                                        
            }
                                , {
                field: 'note_details', width: 180, title: '备注详情', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='student_name']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'sign_in_time', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='sign_in_time']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'note_details', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='note_details']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/class_sign_in/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/class_sign_in/view', 'set') || $check_action('/class_sign_in/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(class_sign_in)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.class_sign_in_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('class_sign_in', BaseUrl + '/api/class_sign_in/del', 'class_sign_in_id', request, resetName, resetSelect        )
