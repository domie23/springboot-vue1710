                                            ...
                                            <div class="layui-input-block">
                                                <input type="text" name="title" lay-verify="title" autocomplete="off"
                                                       ...
                                                       class="layui-input" id="student_name">
                                                                                                                    ...
                                                                                                                    <div class="layui-form-item" id="sign_in_time_box">
                                        <div class="layui-inline">
                                            <label class="layui-form-label">签到时间</label>
                                            <div class="layui-input-inline">
                                                <input type="text" class="layui-input" id="sign_in_time"
                                                       placeholder="yyyy-MM-dd">
                                            </div>
                                        </div>
                                    </div>
                                                                                                    <div class="layui-form-item layui-form-text" id="note_details_box">
                                        <label class="layui-form-label">备注详情</label>
                                        <div class="layui-input-block">
                                            <textarea placeholder="请输入备注详情" class="layui-textarea"
                                                      id="note_details"></textarea>
                                        </div>
                                    </div>
...
<script>
    var BaseUrl = baseUrl()
    let class_sign_in_id = location.search.substring(1)
    layui.use(['upload', 'element', 'layer', 'laydate', 'layedit'], function () {
        ...
        // 提交按钮校验权限
        // if (   user_group == "管理员" ||$check_action('/class_sign_in/view', 'add') || $check_action('/class_sign_in/view', 'set')) {
        //    submit.style.display = "block"
        // }
        // style="display: none"

        
            let field = "class_sign_in_id";
            let url_add = "class_sign_in";
            let url_set = "class_sign_in";
            let url_get_obj = "class_sign_in";
            let url_upload = "class_sign_in"
            let query = {
                "class_sign_in_id": 0,
            }
            ...
            let form_data2 = {
                            ...
                            "sign_in_time":  '', // 签到时间
                            "note_details":  '', // 备注详情
                                    "class_sign_in_id": 0, // ID
                                            }

            layui.layedit.set({
      uploadImage: {
        url: BaseUrl + '/api/class_sign_in/upload?' //接口url
        , type: 'post' //默认post
      }
    });
            ...
            function $check_field(action, field) {
                var o = $get_power("/class_sign_in/view");
                var auth;
                if (o && o[action] != 0 && o[action] != false) {
                    auth = o["field_" + action];
                }
                ...
                return false;
                    ...
                    if ($check_field('add', 'student_name')){
                        ...
                        $("#student_name_box").hide()
                            ...
                            if (user_group === '管理员') {
                    $("#sign_in_time_box").show()
                } else {
                    if ($check_field('add', 'sign_in_time')){
                        $("#sign_in_time_box").show()
                    }else {
                        $("#sign_in_time_box").hide()
                    }
                }
                            if (user_group === '管理员') {
                    $("#note_details_box").show()
                } else {
                    if ($check_field('add', 'note_details')){
                        $("#note_details_box").show()
                    }else {
                        $("#note_details_box").hide()
                    }
                }
                    ...
                    //文本
                                                                                                                                                    //多文本
                    let note_details = document.querySelector("#note_details")
                    //多文本
            ...
            if (class_sign_in_id !== '') {
                async function axios_get_3() {
                    const {data: rese} = await axios.get(BaseUrl + '/api/class_sign_in/get_obj', {
                        params: {
                                class_sign_in_id: class_sign_in_id
                        }, headers: {
                            'x-auth-token': token
                        }
                    })
                    ...
                    Object.keys(form_data2).forEach((key) => {
                        ...
                        $("#"+key).val(form_data2[key])
                            ...
                            if ($check_field('set', 'sign_in_time') || $check_field('get', 'sign_in_time')){
                                $("#sign_in_time_box").show()
                            }else {
                                $("#sign_in_time_box").hide()
                            }
                            ...
                            if ($check_field('set', 'note_details') || $check_field('get', 'note_details')){
                                $("#note_details_box").show()
                            }else {
                                $("#note_details_box").hide()
                            }
                    ...
                    // Array.prototype.slice.call(document.getElementsByTagName('textarea')).map(i => i.disabled = true)
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['class_sign_in_id'] && $check_field('set', 'course_name')) || (!form_data2['class_sign_in_id'] && $check_field('add', 'course_name'))) {
                            }else {
                                $("#course_name").attr("disabled", true);
                                $("#course_name > input[name='file']").attr('disabled', true);
                            }
                                    
                                if (user_group === '管理员' || (form_data2['class_sign_in_id'] && $check_field('set', 'lecturer')) || (!form_data2['class_sign_in_id'] && $check_field('add', 'lecturer'))) {
                            }else {
                                $("#lecturer").attr("disabled", true);
                                $("#lecturer > input[name='file']").attr('disabled', true);
                            }
                                    
                                if (user_group === '管理员' || (form_data2['class_sign_in_id'] && $check_field('set', 'student_users')) || (!form_data2['class_sign_in_id'] && $check_field('add', 'student_users'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['class_sign_in_id'] && $check_field('set', 'student_name')) || (!form_data2['class_sign_in_id'] && $check_field('add', 'student_name'))) {
                            }else {
                                $("#student_name").attr("disabled", true);
                                $("#student_name > input[name='file']").attr('disabled', true);
                            }
                                    
                                if (user_group === '管理员' || (form_data2['class_sign_in_id'] && $check_field('set', 'sign_in_time')) || (!form_data2['class_sign_in_id'] && $check_field('add', 'sign_in_time'))) {
                            }else {
                                $("#sign_in_time").attr("disabled", true);
                                $("#sign_in_time > input[name='file']").attr('disabled', true);
                            }
                                                                //多文本
                                note_details.value = form_data2.note_details
                            //多文本
                        
                                if (user_group === '管理员' || (form_data2['class_sign_in_id'] && $check_field('set', 'note_details')) || (!form_data2['class_sign_in_id'] && $check_field('add', 'note_details'))) {
                            }else {
                                $("#note_details").attr("disabled", true);
                                $("#note_details > input[name='file']").attr('disabled', true);
                            }
                                                                                                                                                                                                                                    sign_in_time.value = layui.util.toDateString(form_data2.sign_in_time, "yyyy-MM-dd HH:mm:ss")
                                                                                            layui.form.render("select");
                }
                axios_get_3()
            }
            ...
            submit.onclick = async function () {
                try {
                            ...
                            //文本
                                                                form_data2.sign_in_time = layui.util.toDateString(form_data2.sign_in_time, "yyyy-MM-dd HH:mm:ss")
                                                                                    //多文本
                            form_data2.note_details = note_details.value
                            //多文本
                                            } catch (err) {
                    console.log(err)
                }
                ...
                let customize_field = []
                            ...
                            "field_name": "签到时间",
                            "field_value": form_data2.sign_in_time,
                            "type": "datetime"
                        });
                                                customize_field.push({"field_name": "备注详情", "field_value": form_data2.note_details});
                    
    
                if (class_sign_in_id == '') {
                                        console.log("新增/Add")
                    const {data: res} = await axios.post(BaseUrl + '/api/class_sign_in/add?',
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
              ...
              layer.msg(res.error.message);
                    ...
                    const {data: res} = await axios.post(BaseUrl + '/api/class_sign_in/set?class_sign_in_id=' + class_sign_in_id,
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
                                    'Content-Type': 'application/json'
